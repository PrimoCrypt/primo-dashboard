...
const Users = ({ params }: { params: { "user-id": number } }) => {
	const { "user-id": id } = params;
	const [searchTerm, setSearchTerm] = useState<string>("");
	const myUserData = usersData.find((idx)=> idx.userId == id)
	const [isLoading, setIsLoading] = useState<boolean>(false);
	const [moreButton, setMoreButton] = useState<boolean>(true);
	const router = useRouter();
	const filteredData = myUserData?.data?.filter(
		(item) =>
			item.date.toLowerCase().includes(searchTerm.toLowerCase()) ||
			item.pickupAddress.toString().toLowerCase().includes(searchTerm.toLowerCase())||
			item.destination.toString().toLowerCase().includes(searchTerm.toLowerCase())||
			...
			<div className="flex flex-col gap-10">
				...
				<div>
					<div className="w-full overflow-y-auto h-full">
						{myUserData?.data && myUserData.data.length > 0 ? (
