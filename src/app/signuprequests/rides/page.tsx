...
import { signUpRequestsCabHailingData } from "@/data";
...
const RidesRequests = () => {
	...
	const [newSignUpRequestsCabHailingData, setNewSignUpRequestsCabHailingData] =
		useState<SignUpRequestsCabHailingDataProps[]>(signUpRequestsCabHailingData);
	const [isLoading, setIsLoading] = useState<boolean>(false);
	const filteredData = newSignUpRequestsCabHailingData.filter((item) =>
		...
		<div className="flex flex-col gap-4">
			<div className="flex flex-col gap-6 sm:gap-10">
				<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
					<h1 className="text-2xl sm:text-3xl font-semibold bg-gradient-to-r from-emerald-400 to-emerald-600 bg-clip-text text-transparent">Ride Applications</h1>
				...
				<div>
					<div className="w-full overflow-y-auto h-full backdrop-blur-sm bg-white/5 rounded-2xl sm:rounded-3xl border border-white/10 shadow-xl p-4 sm:p-6">
						{newSignUpRequestsCabHailingData.length > 0 ? (
							...
							<div className="w-full flex justify-center p-4">
								<h1 className="text-lg sm:text-xl text-gray-400">No Ride Application Data</h1>
							</div>
						)}
					</div>
				</div>
