...
const DeliveriesCustomers = () => {
	const [searchTerm, setSearchTerm] = useState<string>("");
	const [ridersData, setRidersData] =
		useState<RiderDataProps[]>(riderData);
	const [isLoading, setIsLoading] = useState<boolean>(false);
	const filteredData = ridersData.filter((item) =>
					...
					<div className="w-full overflow-y-auto h-full backdrop-blur-sm bg-white/5 rounded-2xl sm:rounded-3xl border border-white/10 shadow-xl p-4 sm:p-6">
						{ridersData.length > 0 ? (
							...
							<div className="w-full flex justify-center p-4">
								<h1 className="text-lg sm:text-xl text-gray-400">No Rider Data</h1>
							</div>
						)}
					</div>
				</div>
...
export default DeliveriesCustomers;
