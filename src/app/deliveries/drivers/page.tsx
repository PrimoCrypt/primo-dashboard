...
const DeliveriesFleet = () => {
	const [searchTerm, setSearchTerm] = useState<string>("");
	const [logisticsDriversData, setLogisticsDriverData] =
		useState<LogisticsDriverDataProps[]>(logisticsDriverData);
	const [isLoading, setIsLoading] = useState<boolean>(false);
	const filteredData = logisticsDriverData.filter((item) =>
		...
		<div className="flex flex-col gap-4 sm:gap-6">
			<div className="flex flex-col gap-6 sm:gap-8">
				<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
					<h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-emerald-400 to-emerald-600 bg-clip-text text-transparent">Recent Deliveries</h1>
					...
					<div className="w-full overflow-y-auto h-full backdrop-blur-sm bg-white/5 rounded-2xl sm:rounded-3xl border border-white/10 shadow-xl p-4 sm:p-6">
						{logisticsDriversData.length > 0 ? (
							...
							<div className="w-full flex justify-center p-4">
								<h1 className="text-lg sm:text-xl text-gray-400">No Driver Data</h1>
							</div>
						)}
					</div>
				</div>
...
export default DeliveriesFleet;
