...
const HangoutsHotel = () => {
	...
	const [myHotelsData, setMyHotelsData] =
		useState<HangoutsHotelsProps[]>(hotelsData);
	const [currentMonth, setCurrentMonth] = useState<string>("");
	const [isLoading, setIsLoading] = useState<boolean>(false);
	const [allMonthsPage, setAllMonthsPage] = useState<boolean>(false);
	

	const filteredData = hotelsData.filter(
		(item) =>
			...
			{allMonthsPage ? (
				<div className="flex flex-col gap-6 sm:gap-10">
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
						<h1 className="text-xl sm:text-2xl font-semibold">All Hotels</h1>
						...
						{allMonths.map((item) => {
							...
							return (
								<Button
									...
									className="bg-background-primo text-white py-2 sm:py-3 rounded-lg sm:rounded-xl text-sm sm:text-lg h-fit px-3 sm:px-6">
									{eachMonth}
								</Button>
							);
						})}
						...
						<div className="w-full overflow-y-auto h-full backdrop-blur-sm bg-white/5 rounded-2xl sm:rounded-3xl border border-white/10 shadow-xl p-4 sm:p-6">
							{myHotelsData.length > 0 ? (
								...
								<div className="w-full flex justify-center p-4">
									<h1 className="text-lg sm:text-xl text-gray-400">No Hotel Data</h1>
								</div>
							)}
						</div>
					</div>
