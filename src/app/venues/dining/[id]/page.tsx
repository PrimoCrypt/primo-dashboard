...
const HangoutsHotel = ({ params }: { params: { id: number } }) => {
	...
	const myRestaurantsData = restaurantsData.find((idx) => idx.id == id);
	const [isLoading, setIsLoading] = useState<boolean>(false);
	const router = useRouter();
	const filteredData =
		myRestaurantsData?.data?.filter(
			(item) =>
				item.noOfBookings
					.toString()
					.toLowerCase()
			...
			<div className="flex flex-col gap-10">
				<div className="flex  items-center justify-between">
					<div className="flex items-center gap-4">
						...
						<h1 className="text-2xl font-semibold">
							{myRestaurantsData?.restaurantName}
						</h1>
					</div>
				...
				<div>
					<div className="w-full overflow-y-auto h-full">
						{myRestaurantsData?.data && myRestaurantsData.data.length > 0 ? (
							<DataTable
