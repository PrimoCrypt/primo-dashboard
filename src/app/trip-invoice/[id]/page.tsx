...
const Customers = ({ params }: { params: { id: number } }) => {
	...
	const myRideData = ridesInvoiceData.find((idx) => idx.rideId == id);
	const [isLoading, setIsLoading] = useState<boolean>(false);
	const [moreButton, setMoreButton] = useState<boolean>(true);
	const router = useRouter();
	const filteredData =
		myRideData?.data?.filter(
			(item) =>
				item.dropOff.toLowerCase().includes(searchTerm.toLowerCase()) ||
				item.date.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
				item.pickUp
					.toString()
					.toLowerCase()
					.includes(searchTerm.toLowerCase()) ||
				item.fare.toString().toLowerCase().includes(searchTerm.toLowerCase())
		) || [];
			...
			<div className="flex flex-col gap-10">
				...
				<div>
					<div className="w-full overflow-y-auto h-full">
						{myRideData?.data && myRideData.data.length > 0 ? (
